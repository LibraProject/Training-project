...
class AddQuestion extends React.Component<Props> {
  ...
  getList = async () => {
    ...
    const list = data.filter((item: any) => item.questions_id === this.props.match.params.id)
      ...
      <Layout className="rankquersition" >
        <h2>编辑试题</h2>
        {
          list.map((item: any,i:number) => {
            return <ul className="rankBox" key={i}>
              <h3>题目信息</h3>
              <li className="li">
                <p>题干</p>
                <p>
                  <Input placeholder="请输入题目标题，不超过20个字" value={item.title} className="ipt" onChange={this.titChange} />
                </p>
              </li>
              <li>
                <p>题目主题</p>
                <div>
                  <Editor value={item.questions_stem} onChange={this.handleChange.bind(this)} />
                </div>
              </li>
              <li className="li">
                <p className="opItem">请选择考试类型</p>
                <Select value={item.exam_name} style={{ width: 130 }} onChange={this.examChange}>
                  {
              ...
              <li className="li">
                ...
                <Select value={item.subject_text} style={{ width: 130 }} onChange={this.subjectChange}>
                  {
              ...
              <li className="li">
                ...
                <Select value={item.questions_type_text} style={{ width: 130 }} onChange={this.questionChange}>
                  {
              ...
              <li>
                ...
                <div>
                  <Editor value={item.questions_answer} onChange={this.handleChangeQuestion.bind(this)} />
                </div>
              </li>
              <li className="li">
